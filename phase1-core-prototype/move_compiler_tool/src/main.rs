...
fn main() -> Result<()> {
    ...
    let targets: Vec<String> = args.sources.iter().map(|p| p.to_string_lossy().to_string()).collect();
    ...
    address_map.insert("std".to_string(), move_compiler::shared::NumericalAddress::parse_str("0x1").unwrap());

    let (files, units_res) = Compiler::from_files::<String, String>(
        targets, 
        deps, 
        address_map,
    )
