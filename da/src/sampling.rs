...
impl DASampler {
    ...
    pub fn calculate_sample_size(confidence: f64, expected_availability: f64) -> usize {
        ...
        if expected_availability <= 0.0 || expected_availability >= 1.0 {
            eprintln!("⚠️  Invalid availability: {}, using 0.75", expected_availability);
...
mod tests {
    ...
    fn test_sampling_full_availability() {
        ...
        let fetcher = |_shard_id: u32| -> Result<Vec<u8>, String> {
            Ok(vec![1, 2, 3, 4])
        };
    ...
    fn test_calculate_sample_size() {
        ...
        assert!(n >= 3 && n <= 10, "Sample size should match formula: got {}", n);
    }
