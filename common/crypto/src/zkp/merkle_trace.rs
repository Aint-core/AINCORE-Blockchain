...
use winterfell::{TraceTable, Trace}; // Trace trait imported but not implemented manually
use winterfell::math::FieldElement; // Import FieldElement for ZERO/ONE
...
impl MerkleTrace {
    ...
    pub fn new(leaf: u64, path: Vec<u64>, path_bits: Vec<bool>) -> Self {
        ...
        assert!(path.len().is_power_of_two(), "path length must be power of 2");
        assert_eq!(path.len(), path_bits.len(), "path and path_bits must have same length");
        ...
        for i in 0..steps {
            ...
            col3.push(if path_bit { BaseElement::ONE } else { BaseElement::ZERO });
